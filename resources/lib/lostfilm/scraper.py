...
class Series(
        namedtuple('Series', [
            'id', 'title', 'original_title', 'image', 'icon', 'poster',
            'country', 'year', 'genres', 'about', 'actors', 'producers',
            'writers', 'plot', 'seasons_count', 'episodes_count'
        ])):
    pass
...
class Episode(
        namedtuple('Episode', [
            ...
            'episode_title', 'original_title', 'release_date', 'icon',
            'poster', 'image'
        ])):
...
class LostFilmScraper(AbstractScraper):
    ...
    def _check_content_is_blocked(self, request, response):
        if request.url in self.anonymized_urls:
            ...
            self.log.info(
                "Content of %s blocked, trying to use anonymous proxy..." %
                request.url)
    ...
    def fetch(self, url, params=None, data=None, **request_params):
        self.response = super(LostFilmScraper, self).fetch(
            url, params, data, **request_params)
    ...
    def authorize(self):
        with Timer(logger=self.log, name='Authorization'):
            ...
            if not action_url or not names or not values:
                self.log.debug(doc)
                raise ScraperError(
                    32003, "Authorization failed", check_settings=True)
    ...
    def get_series_bulk(self, series_ids):
        ...
        if not_cached_ids:
            with Timer(
                    logger=self.log,
                    name="Bulk fetching series with IDs " + ", ".join(
                        str(i) for i in not_cached_ids)):
    ...
    def get_series_episodes_bulk(self, series_ids):
        ...
        with Timer(
                logger=self.log,
                name="Bulk fetching series episodes with IDs " + ", ".join(
                    str(i) for i in series_ids)):
            with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                futures = dict((executor.submit(self.get_series_episodes, _id),
                                _id) for _id in series_ids)
                for future in as_completed(futures):
                    _id = futures[future]
    ...
    def get_series_info(self, series_id):
        ...
        with Timer(
                logger=self.log,
                name='Parsing series info with ID %d' % series_id):
            body = doc.find('div', {'class': 'mid'})
            series_title, original_title = parse_title(
                body.find('h1').first.text)
            image = self.BASE_URL + body.find('img').attr('src')
            ...
            actors = [parse_title(t)
                      for t in res.group(1).split(', ')] if res else None
            ...
            series = Series(series_id, series_title, original_title, image,
                            icon, poster, country, year, genres, about, actors,
                            producers, writers, plot, seasons_count,
                            episodes_count)

            self.log.info(
                "Parsed '%s' series info successfully" % series_title)
    ...
    def browse_episodes(self, skip=0):
        ...
        with Timer(logger=self.log, name='Parsing episodes list'):
            ...
            series_titles = body.find(
                'span', {'style': 'font-family:arial;.*?'}).strings
            ...
            selected_page = body.find('span',
                                      {'class': 'd_pages_link_selected'}).text
            ...
            onclicks = body.find('a', {'href':
                                       'javascript:{};'}).attrs('onClick')
            series_ids, season_numbers, episode_numbers = zip(
                * [parse_onclick(s or "") for s in onclicks])
            posters = [
                poster_url(i[0][18:-5], i[1])
                for i in zip(icons, season_numbers)
            ]
            icons = [self.BASE_URL + url for url in icons]
            images = [
                url.replace('/icons/cat_', '/posters/poster_') for url in icons
            ]
            data = zip(series_ids, series_titles, season_numbers,
                       episode_numbers, episode_titles, original_titles,
                       release_dates, icons, posters, images)
